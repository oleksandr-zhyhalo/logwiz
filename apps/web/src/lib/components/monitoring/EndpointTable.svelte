				...
				<tbody>
					{#each rows as endpoint, index (endpoint.id)}
						<tr class="border-line/40 even:bg-base-200/50 border-b last:border-b-0">
							...
							<td class="text-caption max-w-md py-2 font-mono">
								<div class="flex min-w-0 items-center gap-2" title={endpoint.name}>
									...
									{#if !endpoint.routeAvailable}
										<span
											class="border-warning/30 bg-warning/10 text-warning-ink text-caption shrink-0 rounded border px-1.5 font-sans"
								...
								{#if showService}
									<div class="text-subtle text-caption mt-0.5 truncate" title={endpoint.service}>
										{endpoint.service}
									</div>
								{/if}
							</td>
