...
- Scoped ingest tokens — the index is removed from each token's allowlist. If an allowlist becomes empty, the token is deleted outright so it cannot escalate to all-index scope. Global tokens (no allowlist) are left untouched.
...
- [HTTP Ingestion](/send-logs/http-ingest) — how ingest tokens and the gateway interact with these sources.
