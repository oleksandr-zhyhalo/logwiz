...
<div class="flex items-start gap-3 rounded-lg border border-warning/40 bg-warning/10 px-4 py-3 text-sm">
	...
	<div class="flex-1">
		<p>You need an ingest token before you can send logs.</p>
		<button type="button" class="mt-2 btn btn-sm btn-neutral" onclick={() => (open = true)}>
			Create a token →
		</button>
	</div>
</div>

<CreateTokenModal bind:open indexIds={[DEFAULT_OTEL_LOGS_INDEX_ID]} />
