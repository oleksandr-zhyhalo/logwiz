...
<div class="border-line border-y [&+div]:border-t-0">
	...
	>
		...
		{#if countLabel}
			<span class="text-subtle text-caption">{countLabel}</span>
		{/if}
	</button>
	...
	{#if open}
		<div class="pb-3 {indented ? 'pr-3 pl-6' : 'px-3'}">
			{#if loading && resolvedValues.length === 0}
				<div class="text-subtle text-caption flex items-center gap-2 py-1">
			...
			{:else}
				...
				{#if pinnedVisible.length === 0 && unpinnedVisible.length === 0}
					<p class="text-subtle text-caption py-1">
				...
				{:else}
					{#snippet valueRow(bucket: LogFieldValueBucket)}
						...
						>
							...
							<span
								class="text-subtle text-caption shrink-0 text-right font-sans tabular-nums transition-opacity duration-150 group-focus-within:opacity-0 group-hover:opacity-0"
							>
